...
export const Chatbot: React.FC<ChatbotProps> = ({ rooms, messages, setMessages, onBookingDetected, bookingInProgress }) => {
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error('Dịch vụ AI đang bận, xin vui lòng thử lại sau.');
      }

      const data = await res.json();
      const botReply = data.text || 'Tôi chưa hiểu yêu cầu của quý khách. Bạn có thể nói rõ hơn không?';
      ...
      const errorMessage: Message = {
        id: `m-${Date.now()}-err`,
        role: 'model',
        text: `⚠️   Lỗi kết nối:   ${err.message || 'Không thể liên lạc với máy chủ AI. Vui lòng kiểm tra lại.'}`,
        timestamp: new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }),
      };
