...
- Tiện ích phòng: _${r.description}_`;
  ...
  // 10. Extract customer booking files upload to trigger payment
    ...
    lastUserMsg.includes('cccd') ||
    lastUserMsg.includes('cmnd') ||
    lastUserMsg.includes('cmt') ||
    lastUserMsg.includes('căn cước') ||
    lastUserMsg.includes('can cuoc') ||
    lastUserMsg.includes('sđt') ||
    lastUserMsg.includes('sdt') ||
    lastUserMsg.includes('điện thoại') ||
    lastUserMsg.includes('dien thoai') ||
    lastUserMsg.includes('thông tin') ||
    lastUserMsg.includes('thong tin') ||
    lastUserMsg.includes('hộ chiếu') ||
    lastUserMsg.includes('ho chieu') ||
    lastUserMsg.includes('passport') ||
    lastUserMsg.includes('họ tên') ||
    lastUserMsg.includes('ho ten') ||
    lastUserMsg.includes('tên là') ||
    lastUserMsg.includes('ten la') ||
    lastUserMsg.match(/\b\d{9,12}\b/) || // matches any 9-12 digit string (e.g. CCCD)
    lastUserMsg.match(/\b0\d{9}\b/)      // matches any standard phone number
  ) {
    ...
    const names = lastUserMsg.match(/(nguyễn\s+[a-z\s]+|trần\s+[a-z\s]+|lê\s+[a-z\s]+|vũ\s+[a-z\s]+|phạm\s+[a-z\s]+|họ tên[:\s]+[a-z\s]+|họ và tên[:\s]+[a-z\s]+)/i);
    const guestName = names ? names[0].replace(/họ tên[:\s]+/i, '').replace(/họ và tên[:\s]+/i, '').trim().toUpperCase() : 'NGUYỄN QUÝ KHÁCH';
    const cccds = lastUserMsg.match(/\b\d{9,12}\b/) || ['012356789012'];
    const phones = lastUserMsg.match(/\b(0\d{9,10})\b/) || ['0912345678'];

    // Identify checkin date and checkout date flexibly
    const dateRegex = /\b(\d{4}[-/]\d{2}[-/]\d{2})|(\d{1,2}[-/]\d{1,2}[-/]\d{4})\b/g;
    const dateMatches = lastUserMsg.match(dateRegex) || [];
    
    let checkinMatch = '2026-05-22';
    let checkoutMatch = '2026-05-24';
    ...
    const normalizeDate = (dStr: string): string => {
      ...
      } else if (parts[2] && parts[2].length === 4) { // DD-MM-YYYY or DD/MM/YYYY
    ...
    if (dateMatches.length >= 2) {
      checkinMatch = normalizeDate(dateMatches[0]);
      checkoutMatch = normalizeDate(dateMatches[1]);
    } else if (dateMatches.length === 1) {
      checkinMatch = normalizeDate(dateMatches[0]);
      try {
        const d = new Date(checkinMatch);
        d.setDate(d.getDate() + 2);
        checkoutMatch = d.toISOString().split('T')[0];
      } catch {
        checkoutMatch = '2026-05-24';
      }
    } else {
      const checkinMatchRaw = lastUserMsg.match(/(2026-\d{2}-\d{2})/) || ['2026-05-22'];
      const checkoutMatchRaw = lastUserMsg.match(/(2026-\d{2}-\d{2})/) || ['2026-05-24'];
      checkinMatch = checkinMatchRaw[0];
      checkoutMatch = checkoutMatchRaw[0];
    }

    // Robust Contextual Room Type Retrieval: scan backwards through previous chat history
    let rType = 'Superior Room';
    for (let i = messages.length - 1; i >= 0; i--) {
      const textLower = messages[i].text.toLowerCase();
      if (textLower.includes('presidential')) {
        rType = 'Presidential Suite';
        break;
      }
      if (textLower.includes('executive suite') || (textLower.includes('suite') && !textLower.includes('presidential'))) {
        rType = 'Executive Suite';
        break;
      }
      if (textLower.includes('executive room') || (textLower.includes('executive') && !textLower.includes('suite'))) {
        rType = 'Executive Room';
        break;
      }
      if (textLower.includes('deluxe')) {
        rType = 'Deluxe Room';
        break;
      }
      if (textLower.includes('superior')) {
        rType = 'Superior Room';
        break;
      }
    }

    // Override with any explicit type if the user mentions it again in the current message
    if (lastUserMsg.includes('deluxe')) rType = 'Deluxe Room';
    if (lastUserMsg.includes('executive room') || (lastUserMsg.includes('executive') && !lastUserMsg.includes('suite'))) rType = 'Executive Room';
    ...
    let nights = 1;
    try {
      const d1 = new Date(checkinMatch);
      const d2 = new Date(checkoutMatch);
