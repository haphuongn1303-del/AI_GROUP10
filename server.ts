...
2. Sau khi thu thập đủ các thông tin cá nhân trên:
   ...
   \`\`\`json
   {
     ...
     "roomType": "[Loại phòng chính xác: Superior Room, Deluxe Room, Executive Room, Executive Suite]"
   }
   \`\`\`
...
- Tiện ích phòng: _${r.description}_`;
  ...
  // 9. Process exact booking command or detect booking intent
  else if (lastUserMsg.includes('đặt') || lastUserMsg.includes('superior') || lastUserMsg.includes('deluxe') || lastUserMsg.includes('executive') || lastUserMsg.includes('suite')) {
    ...
    if (lastUserMsg.includes('suite')) detectedRoom = 'Executive Suite';
  ...
  // 10. Extract customer booking files upload to trigger payment
    ...
    if (lastUserMsg.includes('suite')) rType = 'Executive Suite';
